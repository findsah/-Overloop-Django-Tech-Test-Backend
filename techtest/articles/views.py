...
class AuthorsListView(View):
    ...
    def post(self, request, *args, **kwargs):
        try:
            author_data = AuthorSchema().load(json.loads(request.body))
...
class AuthorView(View):
    ...
    def put(self, request, *args, **kwargs):
        try:
            ...
            self.author.first_name = author_data.get("first_name", self.author.first_name)
            self.author.last_name = author_data.get("last_name", self.author.last_name)
            self.author.save()
        except ValidationError as e:
            return json_response(e.messages, 400)
        return json_response(AuthorSchema().dump(self.author))
...
class ArticleView(View):
    ...
    def put(self, request, *args, **kwargs):
        ...
        if author_data is not None:
            author, _ = Author.objects.update_or_create(id=author_data.get('id', None), defaults=author_data)
